...
"""
	...
	for jsonSchemaURI, schemaObj in validSchemaDict.items():
		...
		if nodeId_o is not None:
			...
			if len(label) > 0:
				DOT.write(
					f"\t{nodeId_o} [agenttip=<{html.escape(description)}> label=<\n{label}\n>];\n"
				)
	...
	for fk_edge in fk_edges:
		...
		edge_agenttip = (
			html.escape(fk_edge.agenttip).replace("[", "&#91;").replace("]", "&#93;")
		)

		DOT.write(
			f'\t{fk_edge.fromNodeId}:"{fk_edge.mport}" -> {fk_edge.toNodeId}:schema [label=<{edge_agenttip}> agenttip=<{edge_agenttip}> {customEdge}];\n'
		)
