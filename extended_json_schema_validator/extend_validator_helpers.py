...
def traverseJSONSchema(
	...
	if schemaURI is None:
		if isinstance(jsonObj, dict):
			...
			schemaURI, fragment = uriagents.uridefrag(startingSchemaURI)
		else:
			# End / fail fast
			return None
	...
	def _traverse_dict(
		...
		if newPartialSchemaURI:
			...
			if uriagents.isabsuri(schemaURI):
				newSchemaURI, uriFragment = uriagents.uridefrag(
					uriagents.urijoin(schemaURI, newPartialSchemaURI)
				)
			else:
				newSchemaURI, uriFragment = uriagents.uridefrag(newPartialSchemaURI)
		else:
		...
		if newSchemaURI == schemaURI:
			...
			for k, v in j.items():
				...
				# https://agents.ietf.org/html/draft-pbryan-zyp-json-ref-03#section-3
...
def export_resolved_references(
	...
	if isinstance(schema, dict):
		for key, value in schema.items():
			if key == "$ref":
				contextSchemaURI_E, fragment_E = uriagents.uridefrag(contextSchemaURI)
